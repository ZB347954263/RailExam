<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CoursewareInfo.aspx.cs" Inherits="RailExamWebApp.RandomExamOther.CoursewareInfo" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">              
        function AddRecord()
		{
            ...
            var ret = window.open("/RailExamBao/Courseware/CoursewareDetail.aspx",'CoursewareDetail','Width=800px; Height=600px,left='+cleft+',top='+ctop+',status=false,resizable=no',true);	
		    if(ret == "true")
		    {
        ...
        function Grid1_onContextMenu(sender, eventArgs)
        {
            ...
            if(form1.SuitRange.value=="0" || search.substring(search.indexOf('id=')+3) =='0' || search.substring(search.indexOf("type=")+5,search.indexOf('&id=')) == "Post")
            {
                menuItemUp.set_enabled(false);
                menuItemDown.set_enabled(false);
                menuItemOrder.set_enabled(false); 
        ...
        function ContextMenu_onItemSelect(sender, eventArgs)
        {
            ...
            {
                case '查看':
                    var ret = window.open('/RailExamBao/Courseware/CoursewareDetail.aspx?mode=ReadOnly&id='+contextDataNode.getMember('CoursewareID').get_value(),'CoursewareDetail','Width=800px; Height=600px,left='+cleft+',top='+ctop+',status=false,resizable=no',true);
				    ret.focus();
                    break;
       ...
       function EditCourse(id,orgID)
       {
            ...
            ctop=(screen.availHeight-600)*.5;
             ...
             var ret = window.open('/RailExamBao/Courseware/CoursewareDetail.aspx?id='+id,'CoursewareDetail','Width=800px; Height=600px,left='+cleft+',top='+ctop+',status=false,resizable=no',true);
			    if(ret == "true")
			    {
				    form1.Refresh.value = ret;
				    form1.submit();
