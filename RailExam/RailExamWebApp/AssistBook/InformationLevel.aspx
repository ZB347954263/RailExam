<%@ Page Language="C#" AutoEventWireup="true" Codebehind="InformationLevel.aspx.cs"
    Inherits="RailExamWebApp.AssistBook.InformationLevel" %>
...
<head runat="server">
    ...
    <script type="text/javascript">        
        function tvInformation_onNodeSelect(sender, eventArgs)
        {
            ...
            theItem.value = "";
            theFrame.location = "InformationLevelPreview.aspx?id=" + node.get_id();
...
//                tvInformation.expandAll();
        ...
        function addNode(mode)
        {
            ...
            ctop=(screen.availHeight-315)*.5;   
     	    ...
     	    var url = 'InformationLevelDetail.aspx?mode='+mode+'&id=' + info_id;
     	    var name = 'NewNode';
     	    var features = 'height=330, width=315,left='+cleft+',top='+ctop+', toolbar=no, menubar=no,scrollbars=no, resizable=no, location=no, status=no';
        ...
        function editNode()
        {
            ...
            if(node)
            {
                if(node.get_id())
                {
                    ...
                    var url = 'InformationLevelDetail.aspx?mode=edit&id=' + node.get_id();   // 此处mode参数为固定值edit，用来标记此为修改页。区别于上面方法中的新增操作。
     	            var name = 'EditNode';
     	            var features = 'height=330, width=315,left='+cleft+',top='+ctop+', toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no';
     	            window.open(url,name,features);
                }
...
<body>
    <form id="form1" runat="server">
        <div id="page">
            <div id="head">
                <div id="location1">
                    <div id="desktop1" onclick="window.location='../Main/Desktop.aspx'">
                    </div>
                    <div id="parent1">
                        资料等级管理</div>
                    <div id="separator1">
                ...
                <div id="button">
                    <asp:Button ID="lnkbtnUp" runat="server" CssClass="button" OnClick="lnkbtnUp_Click"
                        Text="上 移"></asp:Button>
                    <asp:Button ID="lnkbtnDown" runat="server" CssClass="button" OnClick="lnkbtnDown_Click"
                        Text="下 移"></asp:Button>
